...
class TrainDataLoader(object):
	...
	def __init__(self, 
		...
		base_file = os.path.abspath(os.path.join(os.path.dirname(__file__), "./Cpp_base/Base.so"))
	...
	def read(self):
		if self.in_path != None:
			self.lib.setInPath(ctypes.create_string_buffer(self.in_path.encode(), len(self.in_path) * 2)) # path: python -> 'C' *char -> 'Cpp' std::string
		else:
